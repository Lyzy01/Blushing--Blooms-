...
════════════════════════════════════════ */
window.previewImage = function(input, previewId, dataId) {
  ...
  reader.onload = function(e) {
    const img = new Image();
    img.onload = function() {
      // Preserve original quality — only resize if extremely large (>1800px)
      const MAX = 1800;
      ...
      preview.innerHTML = `
        <img src="${base64}" style="width:100%;max-height:200px;object-fit:cover;border-radius:10px;">
        <span class="img-upload-text" style="margin-top:8px;">✅ Photo ready · Click to change</span>
      `;
    };
...
function renderProducts(data) {
  ...
  grid.innerHTML = filtered.map((p, i) => `
    <div class="product-card card-animate" style="animation-delay:${i * 0.07}s;${!p.stock ? 'opacity:0.8' : ''}">
      ...
      <div class="product-card-img ${p.image ? 'has-photo' : ''}">
        ${p.image
...
window.renderAdminProducts = function() {
  ...
  list.innerHTML = entries.map(p => `
    <div class="admin-product-item">
      ${p.image
        ? `<img class="api-thumb" src="${p.image}" alt="${p.name}">`
